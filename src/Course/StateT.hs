...
instance Functor k => Functor (StateT s k) where
  (<$>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (<$>) f ma = StateT $ \s -> first f <$> runStateT ma s
...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (<*>) mf ma = StateT $ \s -> runStateT mf s >>= \(f, s') -> runStateT ma s' >>= \(a, s'') -> pure (f a, s'')
...
instance Monad k => Monad (StateT s k) where
  (=<<) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (=<<) f ma = StateT $ \s -> runStateT ma s >>= \(a, s') -> runStateT (f a) s'
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' ma = runExactlyOne . runStateT ma
...
exec' ::
  ...
  -> s
  -> s
exec' ma = runExactlyOne . execT ma
...
eval' ::
  ...
  -> s
  -> a
eval' ma = runExactlyOne . evalT ma
...
distinct' as = eval' (filtering addDistinct as) S.empty
  where addDistinct a = state' $ \s -> if S.member a s then (False, s) else (True, S.insert a s)
...
instance Monad (Logger l) where
  (=<<) ::
    ...
    -> Logger l a
    -> Logger l b
  (=<<) f (Logger la a) = combine (f a)
    where combine (Logger lb b) = Logger (la ++ lb) b
...
distinctG as = runOptionalT $ evalT (filtering addDistinct as) S.empty
  where addDistinct a
          | a > 100 = StateT $ \_ -> OptionalT $ log1 ("aborting > 100: " ++ show' a) Empty
          | even a = StateT $ \s -> OptionalT $ log1 ("even number: " ++ show' a) (merge s a)
          | otherwise = StateT $ \s -> OptionalT $ Logger Nil (merge s a)
        merge s a = pure $ if S.member a s then (False, s) else (True, S.insert a s)
