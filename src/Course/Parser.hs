...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy f = p =<< character
  where p a = if f a then valueParser a else unexpectedCharParser a
...
sequenceParser (ma :. mas) = ma .:. sequenceParser mas
...
smokerParser ::
  Parser Bool
smokerParser = P $ \i -> onResult 
                         (parse (is 'y' ||| is 'n' ||| satisfy (const False)) i)
                         (\i' a -> Result i' (a == 'y'))
