...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  (<$>) f ma = State $ first f . runState ma
...
instance Monad (State s) where
  (=<<) ::
    ...
    -> State s a
    -> State s b
  (=<<) f ma = State $ \s -> let (a, s') = runState ma s in runState (f a) s'
...
isHappy n
  | n <= 0    = False
  | otherwise = contains 1 . firstRepeat $ produce square n
  where square :: Integer -> Integer
        square x = foldRight (+) 0 . map (\x' -> x' * x') . map (toInteger . digitToInt) $ listh (show x)
